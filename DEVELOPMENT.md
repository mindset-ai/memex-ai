...
**Feature hiding (soft-launch control).** The server reads the per-environment `HIDDEN_FEATURES` env var (comma-separated feature slugs) at runtime to suppress UI elements — per-environment, all-or-nothing, fail-open (empty = nothing hidden). The value that **survives a deploy** lives in the GCP Secret Manager secret `memex-<env>-deploy-env`, which CI reads on every deploy (`DEPLOY_CONFIG_SOURCE: secret`); editing the live Cloud Run env var is reverted by the next deploy. Full hide/unhide runbook — slugs, the stores, and the durable change procedure: [`docs/feature-hiding.md`](docs/feature-hiding.md).
