...
// ---------------------------------------------------------------------------
// spec-464 dec-1: traffic-driven phase advancement (spec-189) has been REMOVED.
//
// A Spec's phase now changes ONLY via an explicit publish_spec/update_doc call
// or a human web-UI move — no tool call moves a Spec as a side effect. The pure
// transition function `nextPhaseForTraffic` and its (current × traffic) matrix
// are gone. The manifest classification it used to consume is repurposed as
// `homePhase` (tool-manifest.ts) — the phase in which a tool is in-phase — which
// now drives ahead-of-phase REFUSAL at the tool seam (runToolWithSpecTraffic,
// spec-464 dec-2) instead of silent advancement.

/**
 * How a tool reads against the Spec lifecycle — the phase in which the tool is
 * in-phase (its "home"). Values mirror the SpecPhase names; `null` is
 * never-gated: read-only tools plus lifecycle / narrative / cross-cutting
 * mutators. Consumed by the phase gate, not by any auto-advance path.
 */
export type HomePhase = 'specify' | 'build' | 'verify' | null;
