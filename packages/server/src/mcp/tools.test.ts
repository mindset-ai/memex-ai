...
describe("MCP Tool handlers via HTTP", () => {
  ...
  it("create_doc resolves workspace then calls service", async () => {
    ...
    vi.mocked(getDoc).mockResolvedValue({ groundedStale: false, checkoutHolder: null, supersededByHandle: null, replacesHandles: [], gateOverrides: 0, reopens: 0, ...makeDoc(), sections: [makeSection()] });
  ...
  it("list_docs requires memex via resolver", async () => {
    ...
    expect(listDocs).toHaveBeenCalledWith(TEST_MEMEX_ID, {
      docType: "spec",
      // spec-521 dec-3 (ac-13): the hardcoded `statusIn: ["specify","build","verify"]`
      // is GONE from this call. The handler now fetches the whole set once with
      // includeArchived: true and partitions in memory — that is what lets the
      // response header report honest totals ("N of M; 1 archived, hidden"); you
      // cannot say what you withheld if you never fetched it. Any phase narrowing the
      // caller asks for is applied after the fetch, not pushed into the query.
  ...
  it("update_doc({status}) updates and returns full state (replaces update_doc_status)", async () => {
    ...
    vi.mocked(getDoc).mockResolvedValue({ groundedStale: false, checkoutHolder: null, supersededByHandle: null, replacesHandles: [], gateOverrides: 0, reopens: 0,
  ...
  it("update_section resolves the section via its ref", async () => {
    ...
    vi.mocked(getDoc).mockResolvedValue({ groundedStale: false, checkoutHolder: null, supersededByHandle: null, replacesHandles: [], gateOverrides: 0, reopens: 0, ...makeDoc(), sections: [makeSection({ content: "Updated" })], creator: null });
  ...
  it("list_comments with a doc ref returns the doc-scoped review (replaces list_doc_comments)", async () => {
    vi.mocked(getDoc).mockResolvedValue({ groundedStale: false, checkoutHolder: null, supersededByHandle: null, replacesHandles: [], gateOverrides: 0, reopens: 0, ...makeDoc(), sections: [], creator: null });
