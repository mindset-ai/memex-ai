...
const ALLOWLIST: Record<string, string> = {
  ...
  "services/experiment-sweep.ts":
    "spec-426 verdict sweep. sweepExperimentVerdicts UPDATEs experiment_assignments.outcome/decided_at on the bounded, 3-hourly .unref()'d pass — the same platform-global, RLS-excluded, no-memex_id / no-bus-entity experiment rows as services/experiments.ts. A background drain with no request ALS and no SSE subscriber; routing through mutate() would need a memexId it lacks. Same category as services/usage-forwarder.ts (a background cursor drain over cross-tenant rows). Silent-allowed per std-8 §6.",
  ...
  "services/demo-cleanup.ts":
    "Demo-doc teardown (spec-474 — the residual half of the retired handhold demo; the seed/reset code was deleted). Its DOCUMENT deletes go through mutate() (the per-doc delete loop emits 'document deleted'). The raw writes the scan flags are NON-document log tables with no SSE doc-entity: the synthetic test_events / test_event_latest emissions keyed to the demo ACs (dec-9 — same category as services/test-event-latest.ts), and the activity_log cleanup before the hard-delete (issue-1 / ac-39 — same category as services/activity-log.ts). These are append-only emission/activity logs, not tenant-doc content; they must not emit, exactly like the two log services already allowlisted above.",
  "services/starter-spec-purge.ts":
    "Starter-Spec purge (spec-509 dec-1) — the same teardown shape as services/demo-cleanup.ts above, and allowlisted for the identical reason. Its DOCUMENT deletes go through mutate() (the per-doc loop emits 'document deleted' so live boards refresh, std-8). The raw writes the scan flags are the NON-document log tables the teardown must clear explicitly and IN ORDER: test_events / test_event_latest keyed to the purged Specs' ACs (no docId cascade exists, so they would otherwise strand), and the activity_log rows referencing each doc (brief_id is ON DELETE SET NULL, so leaving them would null the ref and resurface seeded activity in Pulse as memex-level activity). Append-only emission/activity logs, not tenant-doc content — they must not emit.",
};
