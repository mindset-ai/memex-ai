...
import { buildChildRef, buildDocRef, docTypeForUrl } from "../mcp/refs.js";
 ...
 * status (legacy + canonical Spec phases) onto the five-element SpecPhase enum,
 * returning `null` for any unrecognised status. Previously duplicated in
 * `agent/context-builder.ts` (`statusToPhase`), which now delegates here.
 */
export function phaseFromStatus(status: string): SpecPhase | null {
  switch (status) {
    case "draft":
      return "draft";
    case "specify":
