...
export function ResetPassword() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      // their Specs board, a new user on /home. /login is a real SPA path (never the apex
      // `/`, which 301s to marketing). Fall back to the default tenant directly only when
      // 'home' is hidden per-env (mirrors RootRedirect's loop-avoidance).
