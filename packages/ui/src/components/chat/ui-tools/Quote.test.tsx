...
// dispatches 'render_quote' to this component.
...
describe('Quote — copyable handoff prompt (issue-13, ac-6)', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('copyable=true shows the Copy button and writes input.text to the clipboard', async () => {
    ...
    render(<Quote input={{ text: PROMPT, source: 'Prompt for the Standards agent', copyable: true }} />);

    const btn = screen.getByTestId('quote-copy');
  ...
  it('copyable falsy → no Copy button (a plain verbatim quote)', () => {
    ...
    const { rerender } = render(
      <Quote input={{ text: 'verbatim quote', source: 'build phase guidance' }} />,
    );
    expect(screen.queryByTestId('quote-copy')).not.toBeInTheDocument();

    rerender(<Quote input={{ text: 'verbatim quote', source: 'x', copyable: false }} />);
    expect(screen.queryByTestId('quote-copy')).not.toBeInTheDocument();
  });
});

describe('UiToolRenderer dispatch (issue-5, ac-6)', () => {
  it("dispatches 'render_quote' to Quote", () => {
    tagAc(AC);
    const { container } = render(
      <UiToolRenderer
        toolName="render_quote"
        toolId="t1"
